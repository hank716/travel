...
function districtsPrompt(payload: Record<string, unknown>): string {
  ...
  return [
    ...
    "- cc：該地所屬國家的 ISO-3166 alpha-2 兩碼國碼（大寫），例如 台灣→TW、日本→JP、韓國→KR、法國→FR。",
    "  這欄是用來過濾同名地點的，一定要給：像 geo=Jeju 全世界不只一個，沒有 cc=KR 就會查到衣索比亞的同名村莊。",
    "  真的判斷不出國家才留空字串。",
    "",
    "行程：",
    lines,
    "",
    '只輸出 JSON 陣列，格式：[{"date":"YYYY-MM-DD","area":"行政區中文","geo":"RomanizedName","cc":"KR"}]，date 必須與輸入相同。',
  ].join("\n");
}
