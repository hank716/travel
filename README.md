...
- ✅ **新手上手**：❓ 說明頁（分頁導覽 / 隱藏操作 / AI 用法 / 結算邏輯 / FAQ）、空狀態依權限分流、唯讀成員標記、示範行程
...
之後在網站上按「🎌 示範行程」即可 —— 入口有三個：**⚙️ 管理 → 行程管理**、**❓ 說明**頁底部、
以及還沒有行程時**總覽**上的提示。內容是 5 天 24 個行程項目、14 筆三幣別支出（結算有得算）、
17 件行李、4 則備忘與留言、4 位成員（含唯讀）。

出發日固定是「按下按鈕當天 + 7 天」，所以天氣頁一定拿得到預報。重按一次＝砍掉重建，可當重置鈕。
管理員建的是共用的 `DEMO`；一般成員建的是自己專屬那一份（碼是 `DEMO` + uid 前 6 碼，每人限一份，
也砍不到別人的）。
